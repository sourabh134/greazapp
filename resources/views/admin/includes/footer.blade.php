...
<script src="{{ asset('public/vendors/echart/echarts.min.js') }}"></script>
...
{{-- <script>
   function previewFile(input,imheight,imwidth) {
      ...
      if (regex.test(fileUpload.value.toLowerCase())) {
         ...
         if (typeof (fileUpload.files) != "undefined") {
               ...
               reader.onload = function (e) {
                  ...
                  image.onload = function () {
                     ...
                     if (height != imheight || width != imwidth) {
                        ...
                        $('#image_error').text("Invalid image dimensions must be "+imheight+"px * "+imwidth+ "px");
            ...
            $("input[type=file]").val('');
            $('#image_error').text("This browser does not support HTML5.");
         ...
         $('#image_error').text("Please select a valid Image file.");
         return false;
      }
//});
...
<script>
   function previewFiless(input,imheight,imwidth) {
      var files = $("#imagess").get(0).files[0];
      //Get reference of FileUpload.
      var fileUploads = $("#imagess")[0];
      
      //Check whether the file is valid Image.
      var regex = new RegExp("([a-zA-Z0-9\s_\\.\-:])+(.jpg|.png|.gif)$");
      if (regex.test(fileUploads.value.toLowerCase())) {
         //Check whether HTML5 is supported.
         if (typeof (fileUploads.files) != "undefined") {
               //Initiate the FileReader object.
               var reader = new FileReader();
               //Read the contents of Image File.
               ...
               reader.onload = function (e) {
                  ...
                  image.onload = function () {
                     ...
                     if (height != imheight || width != imwidth) {
                        $("#imagess").val('');
                        //alert("Height and Width must not exceed 100px.");
                        $('#images_error').text("Invalid image dimensions must be "+imheight+"px * "+imwidth+ "px");
            ...
            $("#imagess").val('');
            $('#images_error').text("This browser does not support HTML5.");
               //alert("This browser does not support HTML5.");
               return false;
         }
      } else {
         $("#imagess").val('');
         //alert("Please select a valid Image file.");
         $('#images_error').text("Please select a valid Image file.");
         return false;
      }
//});
   ...
   function previewFiles(input) {
      ...
      if (files) {
         ...
         readers.onload = function() {
            $("#previewImgs").attr("src", readers.result);
         }
...
<script>
   function previewFiletag(input,imheight,imwidth) {
      var file = $("#tag_icon").get(0).files[0];
      //Get reference of FileUpload.
      var fileUpload = $("#tag_icon")[0];
      ...
      if (regex.test(fileUpload.value.toLowerCase())) {
         ...
         if (typeof (fileUpload.files) != "undefined") {
               ...
               reader.onload = function (e) {
                  ...
                  image.onload = function () {
                     ...
                     if (height != imheight || width != imwidth) {
                        $("#tag_icon").val('');
                        //alert("Height and Width must not exceed 100px.");
                        $('#tag_icon_error').text("Invalid image dimensions must be "+imheight+"px * "+imwidth+ "px");
                     ...
                     var reader = new FileReader();
                     reader.onload = function() {
                        $("#previewImgtag").attr("src", reader.result);
                     }
                     reader.readAsDataURL(file);
                     return true;
                  };
            ...
            $("#tag_icon").val('');
            $('#tag_icon_error').text("This browser does not support HTML5.");
               //alert("This browser does not support HTML5.");
               return false;
         }
      } else {
         $("#tag_icon").val('');
         //alert("Please select a valid Image file.");
         $('#tag_icon_error').text("Please select a valid Image file.");
         return false;
      }
//});
...
<script>
   function previewFilesponser(input,imheight,imwidth) {
      var file = $("#sponserlogo").get(0).files[0];
      //Get reference of FileUpload.
      var fileUpload = $("#sponserlogo")[0];
      ...
      if (regex.test(fileUpload.value.toLowerCase())) {
         ...
         if (typeof (fileUpload.files) != "undefined") {
               ...
               reader.onload = function (e) {
                  ...
                  image.onload = function () {
                     ...
                     if (height != imheight || width != imwidth) {
                        $("#sponserlogo").val('');
                        //alert("Height and Width must not exceed 100px.");
                        $('#sponser_error').text("Invalid image dimensions must be "+imheight+"px * "+imwidth+ "px");
                     ...
                     var reader = new FileReader();
                     reader.onload = function() {
                        $("#previewImgsponser").attr("src", reader.result);
                     }
                     reader.readAsDataURL(file);
                     return true;
                  };
            ...
            $("#sponserlogo").val('');
            $('#sponser_error').text("This browser does not support HTML5.");
               //alert("This browser does not support HTML5.");
               return false;
         }
      } else {
         $("#sponserlogo").val('');
         //alert("Please select a valid Image file.");
         $('#sponser_error').text("Please select a valid Image file.");
         return false;
      }
//});
