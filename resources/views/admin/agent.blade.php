...
<div class="main_content_iner overly_inner ">
    <div class="container-fluid p-0 ">
        ...
        <div class="col-12">
            ...
            @endif
            <a href="{{ url('admin/addAgent') }}" class="white_btn3">Create</a>
            </div>
        </div>
        </div>
    </div>
                    ...
                    <table class="table p-0" id="example">
                        ...
                        @foreach($data as $value)
                            <tr id="<?php echo $value->id; ?>">
                                ...
                                <td>{{$value->name}}</td>
                                <td>{{$value->name_ar}}</td>
                                <td><?php echo $view = App\Models\Viewed::where('menu_type',3)->where('menuID',$value->id)->count(); ?></td>
                                <td>{{$value->log}}</td>
                                <td>
                                ...
                                <td><a href="{{url('admin/branch?key='.base64_encode($value->id))}}" class="btn btn-warning">ADD</a></td>
                                <td><a href="{{url('admin/addAgent?key='.base64_encode($value->id))}}"><i class="fa fa-edit"></i></a> | <a class="delete" data-id="{{$value->id}}"><i class="fa fa-trash"></i></a></td>
                            </tr>
                        @php
                        $i++;
                        @endphp
...
<script>
    $('.flexSwitchCheckChecked').click(function(){
        ...
        if(confirm("Are you sure want to change this status?")){
            $.ajax({
                type:'POST',
                url:'{{url("/admin/change_agent_status")}}',
...
<script>
    $('.flexSwitchCheckCheckedpopular').click(function(){
        ...
        if(confirm("Are you sure want to add this in popular list?")){
            $.ajax({
                type:'POST',
                url:'{{url("/admin/add_popular_agent")}}',
                data  :{id:id,_token:token,status:status},          
                success:function(data){
                    location.reload();
                }
